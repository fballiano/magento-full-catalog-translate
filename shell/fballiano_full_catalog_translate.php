...
class Fballiano_FullCatalogTranslate_Shell extends Mage_Shell_Abstract
{
    ...
    public function run()
    {
        ...
        $products = Mage::getSingleton("core/resource")->getConnection("core_read")->fetchCol("SELECT entity_id FROM catalog_product_entity_int WHERE attribute_id={$attribute_id} AND store_id={$store_id_dest} AND value=1");
